...
# 安裝依賴
!pip install tensorflow==2.12.0 keras-facenet==0.3.2 mtcnn==0.1.1 deepface==0.0.79 annoy==1.17.3
...
# 列出數據庫內容
print("數據庫文件列表:", os.listdir(f"{BASE_PATH}/database"))
...
# 初始化 FaceNet 模型
print("⏳ 檢查並加載 FaceNet 模型...")
MODEL_PATH = f"{BASE_PATH}/models/facenet_keras.h5"

# 檢查模型是否存在
if not os.path.exists(MODEL_PATH):
    raise FileNotFoundError(f"❌ 模型檔案不存在: {MODEL_PATH}\n請確認你已手動下載 facenet_keras.h5 並放到該路徑。")

# 嘗試加載模型
# 初始化 FaceNet 模型（使用 keras-facenet 內建模型）
print("⏳ 初始化 FaceNet 模型...")
...
if __name__ == "__main__":
    ...
    test_img = "/content/drive/MyDrive/known_faces/gay1.jpg"
    if register.register(test_img, "harrylin", debug=True):
        print("註冊測試通過")
...
test_img2 = "/content/drive/MyDrive/unknown_faces/HarryLin.jpg"#之後可能
print("\n正在進行識別測試...")
result = recognizer.recognize(test_img2, debug=True)
...
img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)  # OpenCV 讀圖為 BGR，需要轉 RGB
plt.imshow(img)
plt.axis('off')
plt.show()
...
!apt-get update
!apt-get install fonts-noto-cjk-tc
!fc-cache -fv